...
def preprocess_fn_train(
    ...
    # pad_on_right = None
    # if isinstance(tokenizer, partial):
    #     if isinstance(tokenizer.args[0], PreTrainedTokenizerBase):
    ...
    cls_token_id = tokenizer.keywords['self'].cls_token_id
...
def preprocess_fn_val_test(
    ...
    # workaround to comply with trident-hydra configuration seemleassly
    # if isinstance(tokenizer, partial) and isinstance(
    #     tokenizer.args[0], PreTrainedTokenizerBase
    # ):
    #     pad_on_right = tokenizer.args[0].padding_side == "right"
    # elif isinstance(tokenizer, PreTrainedTokenizerBase):
    #     pad_on_right = tokenizer.padding_side == "right"
...
def postprocess_qa_predictions(
    ...
    outputs: dict,
    stage: str,
    # examples: Dataset, <- TBD, implement KEEP
    # features: Dataset, <- module.dataset
    ...
    examples = getattr(module.trainer.datamodule, f"dataset_{stage}_raw")
    features = getattr(module.trainer.datamodule, f"dataset_{stage}")
